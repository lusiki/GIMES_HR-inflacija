...
# Izbroji sve pojave korijena riječi "inflacij" u svakom članku
dt[, inflacija_count := stri_count_regex(tolower(FULL_TEXT), "inflacij[a-zčćžšđ]*")]

# Agregiraj po mjesecu
monthly_mentions <- dt[, .(
  total_mentions = sum(inflacija_count, na.rm = TRUE),
  avg_mentions_per_article = mean(inflacija_count, na.rm = TRUE),
  article_count = .N
), by = yearmonth][order(yearmonth)]
...
word_stats <- data.table(
  ...
  Vrijednost = c(
    format(sum(dt$inflacija_count, na.rm = TRUE), big.mark = ","),
    round(mean(dt$inflacija_count, na.rm = TRUE), 2),
