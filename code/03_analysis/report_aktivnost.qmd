...
## Mjesečna agregacija i spajanje s Eurostat podacima

```{r monthly-aggregation-and-merge}
#| label: monthly-aggregation-and-merge
...
# Kreiraj analysis data.table i spoji Eurostat podatke PRIJE konstrukcije indeksa
...
message("Spojeno ", nrow(analysis), " mjeseci podataka")
```
...
# Provjeri da ind_prod_yoy postoji i ima vrijednosti
if("ind_prod_yoy" %in% names(analysis) && sum(!is.na(analysis$ind_prod_yoy)) > 5) {
  correlations_with_ref <- sapply(macro_cols, function(col) {
    if(sum(!is.na(analysis[[col]]) & !is.na(analysis$ind_prod_yoy)) > 5) {
  ...
  # Fallback: koristi SAI kao proxy
  correlations_with_ref <- sapply(macro_cols, function(col) {
    if(sum(!is.na(analysis[[col]])) > 5) {
      cor(analysis[[col]], analysis$semantic_total, use = "complete.obs")
    } else {
      0
    }
  })
...
if(nrow(pca_data_complete) > 10 && ncol(pca_data_complete) > 2) {
  ...
  # Provjeri korelaciju samo ako postoji referentna varijabla
  if("ind_prod_yoy" %in% names(analysis) && sum(!is.na(analysis$ind_prod_yoy)) > 5) {
...
message("Konstruirano 5 semantičkih indeksa")
```
...
# Provjeri dostupnost podataka
has_ind_prod <- "ind_prod_yoy" %in% names(analysis) && sum(!is.na(analysis$ind_prod_yoy)) > 5
...
if(has_ind_prod) {
  ...
  # Fallback: prikaži samo semantičke indekse
  indices_only <- analysis[, .(yearmonth, SAI, WSI, SCI, GDI)]
  indices_long <- melt(indices_only, id.vars = "yearmonth",
                       variable.name = "indeks", value.name = "vrijednost")
  
  ggplot(indices_long, aes(x = yearmonth, y = vrijednost, color = indeks)) +
    geom_line(linewidth = 1) +
    scale_color_manual(values = c(
      "SAI" = pal$cool, "WSI" = pal$success, 
      "SCI" = pal$warm, "GDI" = pal$purple
    )) +
    scale_x_date(date_breaks = "3 months", date_labels = "%b\n%Y") +
    labs(
      title = "Semantički indeksi gospodarske aktivnosti",
      subtitle = "Napomena: Eurostat podaci nisu dostupni za usporedbu",
      x = NULL,
      y = "Indeks (0-100)",
      color = NULL
    ) +
...
# Definiraj pomocne varijable za provjeru
has_ind <- "ind_prod_yoy" %in% names(analysis) && sum(!is.na(analysis$ind_prod_yoy)) > 3
has_con <- "construction_yoy" %in% names(analysis) && sum(!is.na(analysis$construction_yoy)) > 3
has_ret <- "retail_yoy" %in% names(analysis) && sum(!is.na(analysis$retail_yoy)) > 3
has_esi <- "esi" %in% names(analysis) && sum(!is.na(analysis$esi)) > 3

plot_list <- list()

# Industrija
if(has_ind && "macro_industrija" %in% names(analysis)) {
  p_ind <- ggplot(analysis, aes(x = yearmonth)) +
    ...
    theme(legend.position = "bottom")
  plot_list$ind <- p_ind
} else if("macro_industrija" %in% names(analysis)) {
  p_ind <- ggplot(analysis, aes(x = yearmonth, y = macro_industrija)) +
    geom_line(color = pal$cool, linewidth = 1) +
    labs(title = "Industrija (samo semantički)", x = NULL, y = "Broj pojmova")
  plot_list$ind <- p_ind
}

# Građevinarstvo
if(has_con && "macro_gradevinarstvo" %in% names(analysis)) {
  p_con <- ggplot(analysis, aes(x = yearmonth)) +
    ...
    theme(legend.position = "bottom")
  plot_list$con <- p_con
} else if("macro_gradevinarstvo" %in% names(analysis)) {
  p_con <- ggplot(analysis, aes(x = yearmonth, y = macro_gradevinarstvo)) +
    geom_line(color = pal$cool, linewidth = 1) +
    labs(title = "Građevinarstvo (samo semantički)", x = NULL, y = "Broj pojmova")
  plot_list$con <- p_con
}

# Trgovina
if(has_ret && "macro_trgovina" %in% names(analysis)) {
  p_ret <- ggplot(analysis, aes(x = yearmonth)) +
    ...
    theme(legend.position = "bottom")
  plot_list$ret <- p_ret
} else if("macro_trgovina" %in% names(analysis)) {
  p_ret <- ggplot(analysis, aes(x = yearmonth, y = macro_trgovina)) +
    geom_line(color = pal$cool, linewidth = 1) +
    labs(title = "Trgovina (samo semantički)", x = NULL, y = "Broj pojmova")
  plot_list$ret <- p_ret
}

# ESI
if(has_esi && "macro_sentiment_poslovni" %in% names(analysis)) {
  p_esi <- ggplot(analysis, aes(x = yearmonth)) +
    ...
    theme(legend.position = "bottom")
  plot_list$esi <- p_esi
} else if("macro_sentiment_poslovni" %in% names(analysis)) {
  p_esi <- ggplot(analysis, aes(x = yearmonth, y = macro_sentiment_poslovni)) +
    geom_line(color = pal$cool, linewidth = 1) +
    labs(title = "Poslovni sentiment (samo semantički)", x = NULL, y = "Broj pojmova")
  plot_list$esi <- p_esi
}

# Kombiniraj dostupne grafove
if(length(plot_list) >= 4) {
  ...
  ggplot() + 
    annotate("text", x = 0.5, y = 0.5, label = "Nema dostupnih podataka za vizualizaciju") +
    theme_void()
}
```
...
official_vars <- c("ind_prod_yoy", "construction_yoy", "retail_yoy", "esi", "gdp_yoy")
available_official <- official_vars[official_vars %in% names(analysis)]
available_official <- available_official[sapply(available_official, function(v) sum(!is.na(analysis[[v]])) > 3)]

cor_vars <- c(macro_cols, available_official)
...
if(length(cor_vars) > 2) {
  ...
  message("Nedovoljno varijabli za korelacijsku matricu")
}
```
...
calc_performance <- function(idx_col, ref_col) {
  if(!ref_col %in% names(analysis)) return(list(cor = NA, rmse = NA, mae = NA))
  ...
  ref_range <- range(analysis[[ref_col]][complete_idx])
  if(diff(ref_range) == 0) return(list(cor = NA, rmse = NA, mae = NA))
  
  idx_scaled <- normalize_01(analysis[[idx_col]][complete_idx]) * diff(ref_range) + ref_range[1]
...
ref_indicators <- c("ind_prod_yoy", "construction_yoy", "retail_yoy", "esi", "gdp_yoy")
available_refs <- ref_indicators[ref_indicators %in% names(analysis)]
available_refs <- available_refs[sapply(available_refs, function(v) sum(!is.na(analysis[[v]])) > 5)]
...
if(length(available_refs) > 0) {
  ...
  if(nrow(performance_results) > 0) {
    ...
    message("Nema dovoljno podataka za izračun performansi")
  }
} else {
  message("Nema dostupnih referentnih pokazatelja za usporedbu")
  # Kreiraj praznu tablicu za kasniju upotrebu
  best_per_index <- data.table(Indeks = "SAI", Referentni_pokazatelj = NA, 
                                Korelacija = NA, RMSE = NA, MAE = NA)
}
```
...
cat_correlations <- data.table(Kategorija = macro_cols)

ref_indicators <- c("ind_prod_yoy", "construction_yoy", "retail_yoy", "esi", "gdp_yoy")
available_refs <- ref_indicators[ref_indicators %in% names(analysis)]
available_refs <- available_refs[sapply(available_refs, function(v) sum(!is.na(analysis[[v]])) > 3)]

if(length(available_refs) > 0) {
  for(ref in available_refs) {
    cat_correlations[, (ref) := sapply(macro_cols, function(col) {
      if(sum(!is.na(analysis[[col]]) & !is.na(analysis[[ref]])) > 3) {
...
ref_indicators <- c("ind_prod_yoy", "construction_yoy", "retail_yoy", "gdp_yoy")
available_refs <- ref_indicators[ref_indicators %in% names(analysis)]
available_refs <- available_refs[sapply(available_refs, function(v) sum(!is.na(analysis[[v]])) > 10)]
...
if(length(available_refs) > 0) {
  ...
  if(nrow(leading_indicators) > 0) {
    ...
    message("Nema identificiranih leading indikatora")
  }
} else {
  message("Nema dostupnih referentnih pokazatelja za lag analizu")
  leading_indicators <- data.table()
}
```
...
has_ind_prod <- "ind_prod_yoy" %in% names(analysis) && sum(!is.na(analysis$ind_prod_yoy)) > 15
...
if(has_ind_prod) {
  ...
  if(length(ccf_results) > 0) {
    ...
    message("Nedovoljno podataka za CCF analizu")
  }
} else {
  # Prikaži autokorelaciju semantičkih indeksa
  ggplot(analysis, aes(x = yearmonth, y = SAI)) +
    geom_line(color = pal$cool, linewidth = 1) +
    labs(
      title = "SAI indeks (Eurostat podaci nisu dostupni za CCF)",
      x = NULL,
      y = "SAI"
    )
}
...
has_ind_prod <- "ind_prod_yoy" %in% names(analysis) && sum(!is.na(analysis$ind_prod_yoy)) > 20
...
if(has_ind_prod) {
  ...
  if(nrow(model_data) > 20) {
    ...
    # Model 4: AR(1) + sektorski (samo ako postojne potrebne varijable)
    sector_vars <- c("macro_industrija", "macro_trgovina", "macro_sentiment_poslovni")
    available_sector_vars <- sector_vars[sector_vars %in% names(model_data)]
    
    if(length(available_sector_vars) > 0) {
      sector_formula <- as.formula(paste(
        "ind_prod_yoy ~ shift(ind_prod_yoy, 1) +",
        paste(available_sector_vars, collapse = " + ")
      ))
      ...
      model_comparison <- data.table(
        Model = c("AR(1) benchmark", "AR(1) + SAI", "AR(1) + SCI"),
        R2 = c(
          round(summary(model_ar1)$r.squared, 3),
          round(summary(model_sai)$r.squared, 3),
          round(summary(model_sci)$r.squared, 3)
  ...
  message("Eurostat podaci nisu dostupni za nowcasting model")
}
```
...
if(exists("model_sai") && exists("model_data") && nrow(model_data) > 0) {
  ...
  ggplot() +
    annotate("text", x = 0.5, y = 0.5, label = "Eurostat podaci nisu dostupni za nowcasting vizualizaciju") +
    theme_void()
}
```
...
p_vel <- ggplot(analysis[!is.na(SAI_velocity)], aes(x = yearmonth)) +
...
p_mom <- ggplot(analysis[!is.na(SAI_momentum)], aes(x = yearmonth)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = pal$muted) +
...
p_accel <- ggplot(analysis[!is.na(SAI_accel)], aes(x = yearmonth)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = pal$muted) +
...
has_ind_prod <- "ind_prod_yoy" %in% names(analysis) && sum(!is.na(analysis$ind_prod_yoy)) > 30
...
if(has_ind_prod) {
  ...
  if(nrow(complete_data) > 30) {
    ...
    message("Nedovoljno podataka za Granger test")
  }
} else {
  message("Eurostat podaci nisu dostupni za Granger test")
}
```
...
# Pripremi export s dostupnim stupcima
export_cols <- c("yearmonth", "N", "SAI", "WSI", "PCI", "SCI", "GDI",
                 "SAI_velocity", "SAI_momentum")
optional_cols <- c("ind_prod_yoy", "construction_yoy", "retail_yoy", "esi", "gdp_yoy")
available_optional <- optional_cols[optional_cols %in% names(analysis)]
export_cols <- c(export_cols, available_optional)
export_cols <- export_cols[export_cols %in% names(analysis)]

export_main <- analysis[, .SD, .SDcols = export_cols]
names(export_main)[1] <- "Datum"
...
# Sheet 4: Lag analiza
if(exists("lag_results") && nrow(lag_results) > 0) {
...
for(sheet in names(wb)) {
  ncols <- ncol(switch(sheet,
    "Glavni_podaci" = export_main,
    "Sektori" = export_sectors,
    "Performanse" = if(exists("performance_results")) performance_results else data.table(),
    "Lag_analiza" = if(exists("lag_results")) lag_results else data.table(),
    "Korelacije" = cat_correlations,
    data.table()
  ))
  if(ncols > 0) {
...
# Nađi najbolje performanse ako postoje
best_index_name <- "SAI"
best_index_cor <- NA
best_leading_cat <- "N/A"
best_leading_lag <- "N/A"
r2_improvement <- "N/A"

if(exists("best_per_index") && nrow(best_per_index) > 0 && !all(is.na(best_per_index$Korelacija))) {
  best_row <- best_per_index[which.max(Korelacija)]
  best_index_name <- best_row$Indeks
  best_index_cor <- best_row$Korelacija
}

if(exists("leading_indicators") && nrow(leading_indicators) > 0) {
  best_leading_cat <- leading_indicators[1, Semanticka_kategorija]
  best_leading_lag <- paste0(abs(leading_indicators[1, Optimalni_lag]), " mjeseca")
}

if(exists("model_comparison") && !is.null(model_comparison) && nrow(model_comparison) > 1) {
  r2_improvement <- paste0(round((max(model_comparison$R2[-1]) - model_comparison$R2[1]) * 100, 1), " pp")
}
...
summary_findings <- data.table(
  Nalaz = c(
    ...
    "Korelacija s referentnim pokazateljem",
  ...
  Rezultat = c(
    ...
    best_index_name,
    ifelse(is.na(best_index_cor), "N/A", as.character(best_index_cor)),
    best_leading_cat,
    best_leading_lag,
    r2_improvement,
    length(sem_cols),
